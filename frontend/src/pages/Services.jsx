  ...
  return (
    <PageShell testid="page-services">
      <section style={{ paddingTop: 220, paddingBottom: 100, position: "relative" }}>
        <AmbientOrb color="var(--violet)" size={680} blur={140} opacity={0.28} style={{ left: "-8%", top: "10%" }} />
        <div className="container-x" style={{ position: "relative", zIndex: 2 }}>
          <Reveal><span className="coord">Services — Sharp offers, zero packages</span></Reveal>
          <MaskReveal delay={0.1}>
            <h1 className="display" style={{ fontSize: "clamp(72px, 12vw, 220px)", marginTop: 40, lineHeight: 0.86, letterSpacing: "-0.045em" }}>
              What <span className="display-italic" style={{ color: "var(--cobalt)" }}>we build</span>.
            </h1>
          </MaskReveal>
          <MaskReveal delay={0.2}>
            <h1 className="display" style={{ fontSize: "clamp(72px, 12vw, 220px)", lineHeight: 0.86, letterSpacing: "-0.045em", textAlign: "right", color: "var(--ink-mute)" }}>
              Not <span className="display-italic" style={{ color: "var(--violet)" }}>what you asked for</span>.
            </h1>
          </MaskReveal>
          <Reveal delay={0.3}>
          ...
          {services.map((s, i) => (
            <div key={s.num} style={{ padding: "90px 0", borderTop: i > 0 ? "1px solid var(--line)" : "none" }}>
              <div style={{ display: "grid", gridTemplateColumns: "120px 1.4fr 1fr", gap: 40, alignItems: "start" }} className="srv-scene">
                <Reveal>
                  <div>
                    <div className="display display-italic" style={{ fontSize: "clamp(56px, 8vw, 120px)", color: "var(--cobalt)", lineHeight: 0.9 }}>{s.num}</div>
                    <div className="coord" style={{ marginTop: 10, color: "var(--ink-mute)" }}>{s.lens}</div>
                  </div>
                </Reveal>
                <MaskReveal delay={0.1}>
                  <h2 className="display" style={{ fontSize: "clamp(44px, 6vw, 104px)", lineHeight: 0.95, letterSpacing: "-0.035em" }}>{s.name}</h2>
                </MaskReveal>
                <Reveal delay={0.15}>
                  <div>
            ...
            {[
              ...
              <Reveal key={i} delay={i * 0.07}>
                <div style={{ padding: 36, borderRight: i < arr.length - 1 ? "1px solid var(--line)" : "none", background: i === 1 ? "var(--bg-2)" : "transparent", height: "100%" }}>
                  <p className="coord" style={{ color: "var(--cobalt)" }}>{b.k}</p>
          ...
          <Reveal delay={0.1}>
            <div style={{ marginTop: 100, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 24, flexWrap: "wrap" }}>
              <div className="display" style={{ fontSize: "clamp(34px, 5vw, 72px)", maxWidth: "18ch", lineHeight: 1, letterSpacing: "-0.03em" }}>
                Not sure which fits? Tell us what needs to <span className="display-italic" style={{ color: "var(--cobalt)" }}>change</span>.
              </div>
