        ...
        return (
          <section key={p.slug} style={{ paddingTop: 100, paddingBottom: 100, borderTop: "1px solid var(--line)" }} data-testid={`work-card-${p.slug}`}>
            <div className="container-x">
              <Link to={`/work/${p.slug}`} style={{ textDecoration: "none", color: "inherit", display: "block" }} data-cursor data-cursor-label="Enter">
                <div style={{ display: "grid", gridTemplateColumns: flipped ? "1fr 1.5fr" : "1.5fr 1fr", gap: 70, alignItems: "center" }} className="work-chapter">
                  <div style={{ order: flipped ? 2 : 1 }}>
                    <Reveal delay={0.04}>
                      <div style={{ display: "flex", alignItems: "baseline", gap: 20 }}>
                        <span className="display display-italic" style={{ fontSize: "clamp(48px, 6vw, 96px)", color: "var(--silver-blue)", lineHeight: 1 }}>{p.index}</span>
                        <span className="coord">{p.sector}</span>
                      </div>
                    </Reveal>
                    <MaskReveal delay={0.1}>
                      <h2 className="display display-heavy" style={{ fontSize: "clamp(48px, 7vw, 112px)", lineHeight: 0.92, letterSpacing: "-0.035em", marginTop: 20 }}>
                        {p.client}
                      </h2>
                    </MaskReveal>
                    <Reveal delay={0.2}>
                  ...
                  <motion.div style={{ order: flipped ? 1 : 2 }} whileHover={{ scale: 1.015 }} transition={{ duration: 0.9, ease }}>
